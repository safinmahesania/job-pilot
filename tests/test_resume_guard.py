...
REAL_PROFILE = {
    ...
    "skill_categories": [{"label": "Programming", "skills": ["Python"]}],
...
class TestGenerationRefuses:
    ...
    def test_a_fabricated_resume_is_never_returned(self, conn, monkeypatch,
        ...
        monkeypatch.setattr(apply, "extract_requirements", lambda job: ["Sales"])
        ...
        with pytest.raises(FabricationError) as caught:
            apply.generate_resume({"title": "Canada Sales", "company": "PointClickCare",
                                   "description": "Sell healthcare SaaS."})
